...
# Comando: python3 title.ratings.py <input_file> <output_file> <options>
# Options:
...
def convertToJson():
    ...
    with open(in_file) as c_file:
        ...
        for row in reader:
            ...
            average_rating = line.pop("averageRating")
            if not (average_rating == null_value):
                line["average_rating"] = float(average_rating)

            num_votes = line.pop("numVotes")
            if not (num_votes == null_value):
                line["num_votes"] = int(num_votes)
