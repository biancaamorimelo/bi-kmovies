...
# Comando: python3 title.episode.py <input_file> <output_file> <options>
# Options:
...
def convertToJson():
    ...
    with open(in_file) as c_file:
        ...
        for row in reader:
            ...
            line["parent_t_const"] = line.pop("parentTconst")

            #season_number
            season_number = line.pop("seasonNumber")
            if not (season_number == null_value):
                line["season_number"] = int(season_number)

            #episode_number
            episode_number = line.pop("episodeNumber")
            if not (episode_number == null_value):
                line["episode_number"] = int(episode_number)           
