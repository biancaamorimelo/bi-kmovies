...
# Comando: python3 title.principal.py <input_file> <output_file> <options>
# Options:
...
def convertToJson():
    ...
    with open(in_file) as c_file:
        ...
        for row in reader:
            ...
            line["tconst"] = line.pop("tconst")

            ordering = line.pop("ordering")
            if not (ordering == null_value):
                line["ordering"] = int(ordering)

            line["nconst"] = line.pop("nconst")

            line["category"] = line.pop("category")

            job = line.pop("job")
            if not (job == null_value):
                line["job"] = job

            characters = line.pop("characters")
            if not (characters == null_value):
                characters = characters.split('","')
                characters[:] = [x.strip().replace('["','').replace('"]','') for x in characters if not(x == null_value) ]                 
