...
# Comando: python3 title.basics.py <input_file> <output_file> <options>
# Options:
...
def convertToJson():
    ...
    with open(in_file) as c_file:
        ...
        for row in reader:
            ...
            line["title_type"] = line.pop("titleType")
            ...
            if not (end_year == null_value):
                line["end_year"] = int(end_year)

            #runtime_minutes
            runtime_minutes=line.pop("runtimeMinutes")
            if not (runtime_minutes == null_value):
                line["runtime_minutes"] = int(runtime_minutes)

            #genres
            genres = extract(line.pop("genres"))
            if (len(genres) > 0):
                line["genres"] = genres
